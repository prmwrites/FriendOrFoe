class RelationsController < ApplicationController
  ...
  # GET /relations.json
  def index
    @relations = @country.relation.all
  end

  # GET /relations/1
  # GET /relations/1.json
  def show
    @relation = @country.relation.find(params[:id])
  end

  # GET /relations/new
  def new
    @relation = @country.relation.new
  end
  ...
  # POST /relations.json
  def create
    @relation = @conutry.relation.new(params[:relation])

    respond_to do |format|
      if @relation.save
        format.html { redirect_to [@country, @relation], notice: 'Relation was successfully created.' }
  ...
  private
    ...
    def relation_params
      params.require(:relation).permit(:name, :parent_id)
    end
